...
export default function VentasPorSucursalCuenta() {
  ...
  const buscar = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/reportes/ventas-por-sucursal-cuenta?${params.toString()}`);
  ...
  const descargarExcel = () => {
    ...
    exportToExcelMultiHoja(
      ...
      "ventas-por-sucursal-cuenta"
    );
  };

  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Ventas por sucursal y cuenta contable</h1>
      ...
      {pivot && totales && (
        <div className="border border-[var(--color-border)] rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] bg-gray-50">
            <h2 className="font-semibold">Ventas por sucursal y cuenta ({pivot.cuentas.length} cuentas)</h2>
