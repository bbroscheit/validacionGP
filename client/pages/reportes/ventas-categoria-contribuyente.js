...
export default function VentasCategoriaContribuyente() {
  ...
  const buscar = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/reportes/ventas-categoria-contribuyente?${params.toString()}`);
  ...
  // Detalle de líneas de facturación (con su cuenta contable) que arman una categoría -
  // ordenado por cuenta para poder ubicar rápido, por ejemplo, qué cuentas caen en
  // "En Blanco" y deberían tener USERDEF2 cargado.
  ...
  const descargarExcel = () => {
    ...
    exportToExcelMultiHoja(
      ...
      "ventas-categoria-contribuyente"
    );
  };

  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Ventas por categoría y tipo de contribuyente</h1>
      <p className="text-sm text-gray-600 mb-4">
        GL20000 (asientos de ventas, SOURCDOC = SJ) agrupado por categoría de cuenta
        (GL00100.USERDEF2: SERVICIOS, ALQUILERES, etc.) y tipo de contribuyente del cliente
        (RI, CF, EX, Iva No Alcanzado). Quedan afuera solo la cuenta de deudores por ventas
        y las cuentas de impuestos (IVA, IIBB, retenciones) - las cuentas de activo tipo
        &quot;V.A.&quot; sí se incluyen.
      </p>
      ...
      <form onSubmit={buscar} className="flex flex-wrap gap-3 items-end mb-6">
        ...
        <label className="flex items-center gap-2 text-sm pb-1.5">
          ...
          Solo comprobantes fiscales (con &quot;P&quot;: FVPA, FVPB, NCPA...)
        </label>
      ...
      {pivot && (
        <div className="border border-[var(--color-border)] rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] bg-gray-50">
            <h2 className="font-semibold">Ventas por categoría y contribuyente ({pivot.categorias.length} categorías)</h2>
            ...
            <div className="px-4 py-3 border-t border-[var(--color-border)] flex justify-end">
              <button
                onClick={() => exportToExcel(
                  ...
                  `ventas-categoria-${modalCategoria}`
                )}
