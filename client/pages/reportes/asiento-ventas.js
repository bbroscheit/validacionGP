...
export default function AsientoVentas() {
  ...
  useEffect(() => {
    apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/reportes/sucursales-ventas`)
      .then((res) => res.json())
      .then(setSucursales)
      .catch(() => {});
  }, []);
  ...
  const buscar = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/reportes/asiento-ventas?${params.toString()}`);
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Asiento contable de ventas</h1>
      <p className="text-sm text-gray-600 mb-4">
        GL20000 (SOURCDOC = SJ) agrupado por cuenta con Debe y Haber, filtrado opcionalmente
        por sucursal (vía SOP30200.PHONE3). Sin sucursal seleccionada muestra el asiento
        ...
        <div className="border border-[var(--color-border)] rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] bg-gray-50">
            <h2 className="font-semibold">
              Asiento de ventas{sucursal ? ` — ${sucursal}` : " — todas las sucursales"} ({data.rows.length} cuentas)
            </h2>
            <button
