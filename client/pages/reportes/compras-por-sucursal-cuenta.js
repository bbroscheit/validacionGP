...
export default function ComprasPorSucursalCuenta() {
  ...
  const buscar = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/reportes/compras-por-sucursal-cuenta?${params.toString()}`);
  ...
  const descargarExcel = () => {
    ...
    exportToExcelMultiHoja(
      ...
      "compras-por-sucursal-cuenta"
    );
  };

  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Compras por sucursal y cuenta contable</h1>
      <p className="text-sm text-gray-600 mb-4">
        GL20000 (asientos de compras, SOURCDOC = PMTRX/PMVVR) por cuenta contable y zona de
        Contabilidad Analítica, con todas las cuentas (incluida proveedores) sin filtrar
        nada por SQL. Destildá una cuenta para dejarla afuera del total sin sacarla de la vista.
      </p>
      ...
      {pivot && totales && (
        <div className="border border-[var(--color-border)] rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] bg-gray-50">
            <h2 className="font-semibold">Compras por sucursal y cuenta ({pivot.cuentas.length} cuentas)</h2>
