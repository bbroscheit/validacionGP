...
export default function ComprasGL() {
  ...
  const buscar = async (e) => {
    ...
    try {
      ...
      const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/gastos?${params.toString()}`);
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Compras (GL)</h1>
      <p className="text-sm text-gray-600 mb-4">
        GL20000 filtrado por rango de cuentas, mostrando solo SOURCDOC = PMTRX / PMVPY / PMVVR
        (movimientos generados desde una factura de compras, no asientos manuales).
      </p>
      ...
      {data && <ResultTable title="Compras (GL)" rows={data.compras.rows} columns={data.compras.columns} filename="gastos-compras" />}
    </div>
  );
}
